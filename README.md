...
如果没有 `requirements.txt`，可以手动安装主要依赖：
...
├── main.py              # 主程序入口（GUI界面）
├── models.py            # 模型定义（ImprovedUNet, ResNetUNet, TransUNet, DS-TransUNet, SwinUNet等）
...
- **main.py**: 主程序文件，包含 PyQt5 GUI 界面和应用程序入口
- **models.py**: 包含所有模型架构的定义，如 ImprovedUNet、ResNetUNet、TransUNet、DS-TransUNet、SwinUNet 等
- **worker.py**: 包含训练、测试和预测的工作线程类（TrainThread、ModelTestThread、PredictThread）
- **utils.py**: 包含工具函数、数据处理类、数据集类、模型加载函数等辅助功能
...
项目采用模块化设计，将功能拆分为四个主要模块：

- **main.py**: GUI 界面和应用程序主入口
- **models.py**: 所有模型架构的定义
- **worker.py**: 训练、测试、预测的业务逻辑
- **utils.py**: 工具函数、数据处理、模型加载等辅助功能
